...
license: MIT
compatibility: Requires Python 3.10+ and Pillow>=10. No network access needed for core pipeline; image generation uses the host agent's image tool.
metadata:
  version: "2.1.0"
...
> **版本：v2.1.0**（2026-09-04）· 许可证：MIT（见根目录 `LICENSE`）· 变更日志见 README.md
>
> **v2.1.0 精简**：移除 external 扩展模式、self-B 批量与 HTML 可视化面板；`quick_batch.py` 能力并入 `self_skeleton.py`（`--build` 一步拼装，配合 `--auto-creative` 免 LLM 出提示词）；渲染档位仅保留正式档（5.0pro / 2048 / 双稿）；全部输入方式与后处理保留。

产出"唯一确定的主体 ＋ 数量可变的配角（组合档位 1–6 类，也可只有单一主体）＋ 大留白 ＋ 主标题与次要文字（一句短诗 ＋ 季节·拉丁学名两行）"的杂志级静物插画。**用户当次指令永远优先于本技能默认规则。** 支持"先出结构化 brief 规划、再确定性渲染"（self 单一来源，见下文「规划-渲染协同模式」）。
...
> 先规划后渲染（brief@1.1、self 单一来源、校验门、协同 SOP）→ `references/director/`（brief-schema / director-workflow）。
...
- **self 是唯一来源（离线可用）**：本技能按 `references/director/director-workflow.md §2` 自起草 brief，不依赖任何外部模型，迁移到别的电脑照样跑。默认顺滑档（内部起草直接出图、交付附一行 brief 摘要）；用户说"先看方案/先给 brief"则先给方案确认再画。
- **固定下游（self 单源，不维护两套渲染）**：
...
- **批量模式（一批 N 个 brief，默认 10，可选 2/5/10/15/20/30）**：逐个起草 brief 时 LLM 重复输出大量固定字段，批量模式把固定字段交给脚本、LLM 只补创意，token 消耗降到约 1/3。self 单源两条路径：
  - **self-A（推荐，创意最个性）**：`self_skeleton.py` 选题清单→N 份骨架（创意字段 TBD）+ `creative_template.json` → LLM 补 pv_en/poem/latin/title → `self_merge.py` 合并+自动校验；
  - **v2.0 快速（免 LLM）**：`self_skeleton.py --auto-creative --build` 一步完成骨架+自动创意+拼装（原 quick_batch 能力已并入本脚本；可加 `--cast-size`/`--container`/`--skip-validate`）。
  两条路径落地后统一走 `build_from_brief.py --batch <目录>` 一次性校验+拼装全部 A/B 提示词与 overlay 参数，输出紧凑摘要表与批次均衡统计。详见 `director-workflow.md §6`。
- **边界**：brief 只写可变创意，媒介/笔触/色彩总律/边缘/无字等技术段一律本地补；过程 brief/prompt 放项目工作区，规则与脚本只存技能内，final 永久保留、中间稿清理以用户当次约定为准。
...
| 速度 | 恒定正式档 5.0pro/2048/双稿，不因求快降级（步骤7） |
...
| 规划协同 | 先 brief@1.1 后渲染：self 单一来源（本体自起草·离线可用），过 validate_brief 校验门、build_from_brief 拼装；批量走 self-A 或 --auto-creative --build（references/director/） |
...
- `references/director/brief-schema.md`：**brief@1.1 唯一规划格式**（字段/约束/示例）；`director-workflow.md`：执行端唯一 SOP（self 六步起草＋批量路径＋历史去重）
...
- `scripts/self_skeleton.py`：self 批量骨架生成器——选题清单→N 份 brief 骨架（固定字段全填、创意字段 TBD），批次内档位/风格/季节/视角自动均衡，查旬物索引填固有色，输出 creative_template.json 供 LLM 补创意；`--auto-creative` 自动填创意（预制库优先+模板兜底）、`--build` 一步拼装 A/B 提示词+overlay、`--cast-size`/`--container` 可固定档位/容器
